...
const anyProduct: AnyProduct = { name: 'Mr. Fluff', kind: 'British Shorthair', age: 4 };
...
const createKeysFromTitles = (titles:string[] /* */) => titles.map((title) => title.toLowerCase().replace(' ', '_').replace('?', ''));
...
const getTax = (course: Course) => {
  typeof course.price === 'string' ? Number(course.price) * 0.25 : course.price * 0.25;
};
...
const fetchAccount = (id: number): object => ({ id, name: 'Some Account', currency: 'USD' });
const currentAccount = fetchAccount(4) as Account; /* add Type Assertion here */
